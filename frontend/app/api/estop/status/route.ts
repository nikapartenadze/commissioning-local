...
export async function GET(req: Request, res: Response) {
  try {
    ...
    const { hasMcm } = await import('@/lib/mcm-registry')
    ...
    // Read every bucketed tag: registry MCMs via the mode-aware typed batch,
    // legacy zones via the singleton cached-read path. Shared with the safety
    // and network status routes (lib/plc/read-bool-tags.ts) — same value
    // semantics + per-tag diagnostics this route surfaces below as tagIssues.
    const { values: tagValues, diag: tagDiag, errors: tagErrors, connectedSids, anyConnected } =
