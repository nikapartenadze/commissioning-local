 ...
 * then commits the pass/fail observation here. Per TSK-2055 the result is
 * binary (pass/fail) plus optional comments.
 *
 * Body: { subsystemId, zoneName, checkTag, result: 'pass' | 'fail',
 *         comments?: string, testedBy?: string, reset?: boolean }
 *
 * If `reset: true`, the row is cleared back to null (Result, Comments,
 * TestedBy, TestedAt all NULL) but the row itself stays so Version keeps
 * incrementing — useful for future cloud-sync.
 *
 * Identity is the composite (SubsystemId, ZoneName, CheckTag), NOT the
 * EStopEpcs.id, because the cloud-pull route DELETEs and re-inserts EStop
 * rows on every refresh.
 */

const upsertStmt = db.prepare(`
  INSERT INTO EStopEpcChecks (SubsystemId, ZoneName, CheckTag, Result, Comments, TestedBy, TestedAt, Version, CreatedAt, UpdatedAt)
...
export async function POST(req: Request, res: Response) {
  try {
    const { subsystemId, zoneName, checkTag, result, comments, testedBy, reset } = req.body as {
      subsystemId?: number
      zoneName?: string
      checkTag?: string
      result?: 'pass' | 'fail'
    ...
    return res.json({ success: true, result, testedBy: testedBy ?? null })
  } catch (error) {
