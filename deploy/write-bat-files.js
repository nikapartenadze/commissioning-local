...
writeBat('START.bat', [
  ...
  ')',
  '',
  'REM Stop the IOCheckout Windows service if it is running',
  'sc query IOCheckout >nul 2^>^&1',
  'if %errorlevel% equ 0 (',
  '    echo Stopping IOCheckout service...',
  '    net stop IOCheckout >nul 2^>^&1',
  '    if %errorlevel% neq 0 (',
  '        echo   Need admin rights to stop the service. Requesting elevation...',
  '        powershell -NoProfile -Command "Start-Process cmd -ArgumentList \'/c net stop IOCheckout\' -Verb RunAs -Wait" 2>nul',
  '    )',
