...
This installs all dependencies and auto-generates the Prisma client (`postinstall` hook).
...
- **PLC WebSocket server** on port `3001` (broadcasts real-time tag state changes to browsers)
...
| `npm run dev:ws` | Starts only the WebSocket server on port 3001 |
...
cd frontend && npm run test:plc

# Quick connection test
cd frontend && npm run test:plc:simple
```
...
4. Run `SETUP-FIREWALL.bat` **as Administrator** (one-time — opens ports 3000 and 3001)
...
| Script | What it does |
|--------|-------------|
| `START.bat` | Starts the app (Next.js + WebSocket on ports 3000/3001) |
...
| 3001 | WebSocket | Browsers (real-time PLC state updates, auto-connected) |
